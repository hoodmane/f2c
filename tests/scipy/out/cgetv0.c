...
/*     %--------------------------------------% */
    ...
    if (msglvl > 2) {
	svout_(&logfil, &c__1, &rnorm0, &ndigit, "_getv0: re-orthonalization"
		" ; rnorm0 is", (ftnlen)38);
	svout_(&logfil, &c__1, rnorm, &ndigit, "_getv0: re-orthonalization ;"
		" rnorm is", (ftnlen)37);
    }
...
L50:
    ...
    if (msglvl > 0) {
	svout_(&logfil, &c__1, rnorm, &ndigit, "_getv0: B-norm of initial / "
		"restarted starting vector", (ftnlen)53);
    }
    if (msglvl > 2) {
	cvout_(&logfil, n, &resid[1], &ndigit, "_getv0: initial / restarted "
