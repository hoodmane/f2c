	...
	real *q, integer *ldq, real *workd)
{
    ...
    extern /* Subroutine */ int sscal_(integer *, real *, real *, integer *), 
	    ...
	    real *, integer *, real *, real *, integer *), scopy_(integer *, 
	    real *, integer *, real *, integer *), saxpy_(integer *, real *, 
	    real *, integer *, real *, integer *), ivout_(integer *, integer *
	    , integer *, integer *, char *, ftnlen), svout_(integer *, 
    ...
    extern /* Subroutine */ int slacpy_(char *, integer *, integer *, real *, 
	    integer *, real *, integer *), slartg_(real *, real *, real *, 
	    real *, real *), slaset_(char *, integer *, integer *, real *, 
	    real *, real *, integer *);
    static integer msapps, msglvl, istart;
    static real tsapps;
...
/*        %------------------------------------------------% */
	...
	for (i__ = istart; i__ <= i__2; ++i__) {
	    big = (r__1 = h__[i__ + (h_dim1 << 1)], abs(r__1)) + (r__2 = h__[
		    i__ + 1 + (h_dim1 << 1)], abs(r__2));
	    if (h__[i__ + 1 + h_dim1] <= epsmch * big) {
		if (msglvl > 0) {
		    ivout_(&logfil, &c__1, &i__, &ndigit, "_sapps: deflation"
			    " at row/column no.", (ftnlen)35);
		    ivout_(&logfil, &c__1, &jj, &ndigit, "_sapps: occured be"
			    "fore shift number.", (ftnlen)36);
		    svout_(&logfil, &c__1, &h__[i__ + 1 + h_dim1], &ndigit, 
...
/*     %------------------------------------------% */
	...
	big = (r__1 = h__[i__ + (h_dim1 << 1)], abs(r__1)) + (r__2 = h__[i__ 
		+ 1 + (h_dim1 << 1)], abs(r__2));
	if (h__[i__ + 1 + h_dim1] <= epsmch * big) {
	    if (msglvl > 0) {
		ivout_(&logfil, &c__1, &i__, &ndigit, "_sapps: deflation at "
			"row/column no.", (ftnlen)35);
		svout_(&logfil, &c__1, &h__[i__ + 1 + h_dim1], &ndigit, "_sa"
...
/*     %-------------------------------------% */
    ...
    if (msglvl > 1) {
	svout_(&logfil, &c__1, &q[kplusp + *kev * q_dim1], &ndigit, "_sapps:"
		" sigmak of the updated residual vector", (ftnlen)45);
	svout_(&logfil, &c__1, &h__[*kev + 1 + h_dim1], &ndigit, "_sapps: be"
		"tak of the updated residual vector", (ftnlen)44);
	svout_(&logfil, kev, &h__[(h_dim1 << 1) + 1], &ndigit, "_sapps: upda"
		"ted main diagonal of H for next iteration", (ftnlen)53);
	if (*kev > 1) {
	    i__1 = *kev - 1;
	    svout_(&logfil, &i__1, &h__[h_dim1 + 2], &ndigit, "_sapps: updat"
