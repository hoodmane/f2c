...
/* Subroutine */ int idd_permmult__(integer *m, integer *ind, integer *n, 
	integer *indprod)
{
...
/* Subroutine */ int idd_qinqr__(integer *m, integer *n, doublereal *a, 
...
/* k */
	...
	for (j = k; j <= i__1; ++j) {
	    ...
	    ifrescal = 1;
	    if (k < *m) {
		idd_houseapp__(&mm, &a[k + 1 + k * a_dim1], &q[k + j * q_dim1]
...
/* Subroutine */ int idd_qmatvec__(integer *iftranspose, integer *m, integer *
...
/*       v -- vector to which Q (or its transpose) has been applied */
	...
	for (k = *krank; k >= 1; --k) {
	    mm = *m - k + 1;
	    if (k < *m) {
		idd_houseapp__(&mm, &a[k + 1 + k * a_dim1], &v[k], &ifrescal, 
			&scal, &v[k]);
	    }
	}
...
/* Subroutine */ int idd_qmatmat__(integer *iftranspose, integer *m, integer *
...
/*         calculating all scals (ifrescal = 1). */
	...
	for (k = *krank; k >= 1; --k) {
	    if (k < *m) {
		mm = *m - k + 1;
		idd_houseapp__(&mm, &a[k + 1 + k * a_dim1], &b[k + j * b_dim1]
...
/*           using the scals just computed (ifrescal = 0). */
		...
		for (k = *krank; k >= 1; --k) {
		    if (k < *m) {
			mm = *m - k + 1;
			idd_houseapp__(&mm, &a[k + 1 + k * a_dim1], &b[k + j *
...
/* Subroutine */ int iddp_qrpiv__(doublereal *eps, integer *m, integer *n, 
...
/*           of the Householder transformation). */
	...
	idd_house__(&mm, &a[*krank + *krank * a_dim1], &a[*krank + *krank * 
...
/*           with upper leftmost entry at position (krank,krank+1). */
	...
	if (*krank < *n) {
		...
		idd_houseapp__(&mm, &a[*krank + 1 + *krank * a_dim1], &a[*
...
/* Subroutine */ int iddr_qrpiv__(integer *m, integer *n, doublereal *a, 
...
/*           of the Householder transformation). */
	    ...
	    idd_house__(&mm, &a[loop + loop * a_dim1], &a[loop + loop * 
...
/*           with upper leftmost entry at position (loop,loop+1). */
		...
		for (k = loop + 1; k <= i__2; ++k) {
		    idd_houseapp__(&mm, &a[loop + 1 + loop * a_dim1], &a[loop 
