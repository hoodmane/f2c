...
/* Subroutine */ int dnaitr_(integer *ido, char *bmat, integer *n, integer *k,
	...
	ipntr, doublereal *workd, integer *info, ftnlen bmat_len)
{
    ...
    extern /* Subroutine */ int ivout_(integer *, integer *, integer *, 
	    ...
	    ftnlen), dlabad_(doublereal *, doublereal *);
...
L80:
	...
	xtemp[0] = wnorm;
	xtemp[1] = *rnorm;
	dvout_(&logfil, &c__2, xtemp, &ndigit, "_naitr: re-orthonalization; "
		"wnorm and rnorm are", (ftnlen)47);
	dvout_(&logfil, &j, &h__[j * h_dim1 + 1], &ndigit, "_naitr: j-th col"
		"umn of H", (ftnlen)24);
    }
...
/*        %-----------------------------------------------------% */
	...
	if (msglvl > 2) {
	    xtemp[0] = *rnorm;
	    xtemp[1] = rnorm1;
	    dvout_(&logfil, &c__2, xtemp, &ndigit, "_naitr: iterative refine"
...
/* L110: */
	...
	if (msglvl > 2) {
	    i__1 = *k + *np;
	    i__2 = *k + *np;
	    dmout_(&logfil, &i__1, &i__2, &h__[h_offset], ldh, &ndigit, "_na"
