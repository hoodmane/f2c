	...
	integer *np, real *ritz, real *bounds, real *shifts, ftnlen which_len)
{
    ...
    extern /* Subroutine */ int sswap_(integer *, real *, integer *, real *, 
	    ...
	    ftnlen), arscnd_(real *);
    static integer logfil, ndigit, msgets, msglvl;
    static real tsgets;
...
/*        %-------------------------------------------------------% */
    ...
    if (msglvl > 0) {
	ivout_(&logfil, &c__1, kev, &ndigit, "_sgets: KEV is", (ftnlen)14);
	ivout_(&logfil, &c__1, np, &ndigit, "_sgets: NP is", (ftnlen)13);
	i__1 = *kev + *np;
	svout_(&logfil, &i__1, &ritz[1], &ndigit, "_sgets: Eigenvalues of cu"
		"rrent H matrix", (ftnlen)39);
	i__1 = *kev + *np;
	svout_(&logfil, &i__1, &bounds[1], &ndigit, "_sgets: Associated Ritz"
		" estimates", (ftnlen)33);
    }
