	...
	workd, integer *info, ftnlen bmat_len)
{
    ...
    static integer logfil, ndigit, mcaitr;
...
L1000:
    ...
    if (msglvl > 1) {
	ivout_(&logfil, &c__1, &j, &ndigit, "_naitr: generating Arnoldi vect"
		"or number", (ftnlen)40);
	svout_(&logfil, &c__1, rnorm, &ndigit, "_naitr: B-norm of the curren"
...
/*           %---------------------------------------------------% */
    ...
    if (msglvl > 0) {
	ivout_(&logfil, &c__1, &j, &ndigit, "_naitr: ****** RESTART AT STEP "
		"******", (ftnlen)37);
    }
...
/*        %-------------------------------------% */
    ...
    if (*(unsigned char *)bmat == 'G') {
	wcdotc_(&q__1, n, &resid[1], &c__1, &workd[ipj], &c__1);
...
L80:
	...
	rtemp[0] = wnorm;
	rtemp[1] = *rnorm;
	svout_(&logfil, &c__2, rtemp, &ndigit, "_naitr: re-orthogonalization"
		"; wnorm and rnorm are", (ftnlen)49);
	cvout_(&logfil, &j, &h__[j * h_dim1 + 1], &ndigit, "_naitr: j-th col"
		"umn of H", (ftnlen)24);
    }
...
/*        %-----------------------------------------------------% */
    ...
    if (msglvl > 0 && iter > 0) {
	ivout_(&logfil, &c__1, &j, &ndigit, "_naitr: Iterative refinement fo"
		"r Arnoldi residual", (ftnlen)49);
	if (msglvl > 2) {
	    rtemp[0] = *rnorm;
	    rtemp[1] = rnorm1;
	    svout_(&logfil, &c__2, rtemp, &ndigit, "_naitr: iterative refine"
...
/* Computing MAX */
	    r__3 = ulp * tst1;
	    if (slapy2_(&r__1, &r__2) <= max(r__3,smlnum)) {
...
/* L110: */
	...
	if (msglvl > 2) {
	    i__1 = *k + *np;
	    i__2 = *k + *np;
	    cmout_(&logfil, &i__1, &i__2, &h__[h_offset], ldh, &ndigit, "_na"
