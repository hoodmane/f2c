...
static doublecomplex c_b16 = {.5,0.};
static doublereal c_b56 = .5;
static doublereal c_b57 = 1.5;
static integer c_n1 = -1;
static doublereal c_b155 = 3.;
static doublereal c_b227 = .33333333333333331;
static integer c__200 = 200;
static integer c__15 = 15;
static real c_b323 = 2.f;
static doublereal c_b329 = 1.27;
static integer c__0 = 0;
static doublereal c_b530 = 1.;
static doublecomplex c_b777 = {-.125,0.};
static doublecomplex c_b781 = {.375,0.};
static integer c__1 = 1;
static doublecomplex c_b1167 = {-2.,0.};
static doublereal c_b1174 = .33333;
static doublecomplex c_b1400 = {-1.,0.};
...
/*          Only one constant has to be adapted. */
	if (d_imag(z__) > -z__->r && d_imag(z__) <= z__->r) {
/*            right quadrant */
	    d__.r = .5, d__.i = 0.;
	} else if (d_imag(z__) > z__->r && d_imag(z__) >= -z__->r) {
/*            upper quadrant */
	    d__.r = 0., d__.i = -.5;
	} else if (d_imag(z__) < -z__->r && d_imag(z__) >= z__->r) {
/*            left quadrant */
...
/*       ============================================================= */
	...
	d__1 = (doublereal) (*nt);
	d__2 = (doublereal) (*nt);
	xm = pow_dd(&d__1, &c_b56) * 2.248485f - 1.f - *nt * .0159382f + 
		pow_dd(&d__2, &c_b57) * 3.208775e-4f;
	nm = (integer) (*nt * .05875f + 14.5f);
	mm = (integer) (*nt * .02f) + 6;
    } else {
	d__1 = (doublereal) (*nt);
	d__2 = (doublereal) (*nt);
	xm = pow_dd(&d__1, &c_b56) * 1.445389f + 5.f + *nt * .01889876f - 
		pow_dd(&d__2, &c_b57) * 2.147763e-4f;
	...
	x1 = pow_dd(&d__1, &c_b56) * .4795504f + .407658f + (i__ - 1) * 
		.983618f;
	d__1 = (doublereal) (i__ - 1);
	x2 = pow_dd(&d__1, &c_b56) * .8333883f + 1.99535f + (i__ - 1) * 
		.984584f;
	l1 = 0;
	i__2 = mm;
	for (j = 1; j <= i__2; ++j) {
...
/*       ==================================================== */
    ...
    if (*m == 0) {
	if (*q <= 1.f) {
	    *a0 = (((q2 * .0036392 - .0125868) * q2 + .0546875) * q2 - .5) * 
		    q2;
	} else if (*q <= 10.f) {
	    *a0 = ((*q * .003999267 - .09638957) * *q - .88297) * *q + 
		    .5542818;
	} else {
	    cvql_(kd, m, q, a0);
	}
	...
	if (*q <= 1.f && *kd == 2) {
	    *a0 = (((*q * -6.51e-4f - .015625) * *q - .125) * *q + 1.f) * *q 
		    + 1.f;
	} else if (*q <= 1.f && *kd == 3) {
	    *a0 = (((*q * -6.51e-4f + .015625) * *q - .125) * *q - 1.f) * *q 
		    + 1.f;
	} else if (*q <= 10.f && *kd == 2) {
	    *a0 = (((*q * -4.94603e-4 + .0192917) * *q - .3089229) * *q + 
		    1.33372) * *q + .811752;
	} else if (*q <= 10.f && *kd == 3) {
	...
	if (*q <= 1.f && *kd == 1) {
	    *a0 = (((q2 * -.0036391 + .0125888) * q2 - .0551939) * q2 + 
		    .416667) * q2 + 4.;
	} else if (*q <= 1.f && *kd == 4) {
	    *a0 = (q2 * 3.617e-4 - .0833333) * q2 + 4.f;
	} else if (*q <= 15. && *kd == 1) {
	    *a0 = (((*q * 3.200972e-4 - .008667445) * *q - 1.829032e-4) * *q 
		    + .9919999) * *q + 3.3290504;
	} else if (*q <= 10.f && *kd == 4) {
	    *a0 = ((*q * .00238446 - .08725329) * *q - .004732542) * *q + 
		    4.00909;
	} else {
	    cvql_(kd, m, q, a0);
	}
	...
	if (*q <= 1.f && *kd == 2) {
	    *a0 = ((*q * 6.348e-4f + .015625) * *q + .0625) * q2 + 9.f;
	} else if (*q <= 1.f && *kd == 3) {
	    *a0 = ((*q * 6.348e-4f - .015625) * *q + .0625) * q2 + 9.f;
	} else if (*q <= 20.f && *kd == 2) {
	    *a0 = (((*q * 3.035731e-4 - .01453021) * *q + .19069602) * *q - 
		    .1039356) * *q + 8.9449274;
	} else if (*q <= 15.f && *kd == 3) {
	    *a0 = ((*q * 9.369364e-5 - .03569325) * *q + .2689874) * *q + 
		    8.771735;
	} else {
	    cvql_(kd, m, q, a0);
	}
	...
	if (*q <= 1.f && *kd == 1) {
	    *a0 = ((q2 * -2.1e-6f + 5.012e-4f) * q2 + .0333333) * q2 + 16.f;
	} else if (*q <= 1.f && *kd == 4) {
	    *a0 = ((q2 * 3.7e-6f - 3.669e-4f) * q2 + .0333333) * q2 + 16.f;
	} else if (*q <= 25.f && *kd == 1) {
	    *a0 = (((*q * 1.076676e-4 - .0079684875) * *q + .17344854) * *q - 
		    .5924058) * *q + 16.620847;
	} else if (*q <= 20.f && *kd == 4) {
	    *a0 = ((*q * -7.08719e-4 + .0038216144) * *q + .1907493) * *q + 
		    15.744;
	} else {
	    cvql_(kd, m, q, a0);
	}
	...
	if (*q <= 1.f && *kd == 2) {
	    *a0 = ((*q * 6.8e-6f + 1.42e-5f) * q2 + .0208333) * q2 + 25.f;
	} else if (*q <= 1.f && *kd == 3) {
	    *a0 = ((*q * -6.8e-6f + 1.42e-5f) * q2 + .0208333) * q2 + 25.f;
	} else if (*q <= 35.f && *kd == 2) {
	    *a0 = (((*q * 2.238231e-5 - .002983416) * *q + .10706975) * *q - 
		    .600205) * *q + 25.93515;
	} else if (*q <= 25.f && *kd == 3) {
	...
	if (*q <= *m * 3.f) {
	    ...
	    if (*m == 8 && *kd == 1) {
		*a0 = (((*q * 8.634308e-6 - .002100289) * *q + .169072) * *q 
			- 4.64336) * *q + 109.4211;
	    } else if (*m == 8 && *kd == 4) {
		*a0 = ((*q * -6.7842e-5 + .0022057) * *q + .48296) * *q + 
			56.59;
	    } else if (*m == 9 && *kd == 2) {
		*a0 = (((*q * 2.906435e-6 - .001019893) * *q + .1101965) * *q 
			- 3.821851) * *q + 127.6098;
	    } else if (*m == 9 && *kd == 3) {
		*a0 = ((*q * -9.577289e-5 + .01043839) * *q + .06588934) * *q 
			+ 78.0198;
	    } else if (*m == 10 && *kd == 1) {
		*a0 = (((*q * 5.44927e-7 - 3.926119e-4) * *q + .0612099) * *q 
			- 2.600805) * *q + 138.1923;
	    } else if (*m == 10 && *kd == 4) {
		*a0 = ((*q * -7.660143e-5 + .01132506) * *q - .09746023) * *q 
			+ 99.29494;
	    } else if (*m == 11 && *kd == 2) {
		*a0 = (((*q * -5.67615e-7 + 7.152722e-6) * *q + .01920291) * *
			q - 1.081583) * *q + 140.88;
	    } else if (*m == 11 && *kd == 3) {
		*a0 = ((*q * -6.310551e-5 + .0119247) * *q - .2681195) * *q + 
			123.667;
	    } else if (*m == 12 && *kd == 1) {
		*a0 = (((*q * -2.38351e-7 - 2.90139e-5) * *q + .02023088) * *
			q - 1.289) * *q + 171.2723;
	    } else if (*m == 12 && *kd == 4) {
		*a0 = (((*q * 3.08902e-7 - 1.577869e-4) * *q + .0247911) * *q 
			- 1.05454) * *q + 161.471;
	    }
	}
    }
...
L55:
	...
	*id = 10;
    ...
    b0 = *kd * *m / pow_dd(x, &c_b155) / (1.f - *kd / (*x * *x)) * *r2f;
    sud = 0.;
    eps2 = 0.;
    i__1 = nm;
    for (k = 1; k <= i__1; ++k) {
...
/*       ======================================================== */
	...
	x = x1 - (x1 - x0) / (1. - f0 / f1);
	cvf_(kd, m, q, &x, &mj, &f);
	if ((d__1 = 1. - x1 / x, abs(d__1)) < eps || f == 0.f) {
	    goto L15;
	}
	x0 = x1;
	f0 = f1;
...
/*             DLMF 9.9.18 */
	    d__1 = u * u;
	    rt0 = -pow_dd(&d__1, &c_b227) * (u1 * (u1 * (u1 * (u1 * 
...
/* L65: */
    ...
    sa0 = ((*m + ip) * 2.f + 1.f) * r1 / (pow_ri(&c_b323, n) * pow_di(c__, &
	    ip) * r2 * r3 * df[1]);
    *ck1 = sa0 * su0;
    if (*kd == -1) {
	return 0;
...
/*       ========================================================= */
	...
	if (nr > 1) {
	    d__1 = (doublereal) nr;
	    z__ = x[nr - 1] + hn * pow_dd(&d__1, &c_b329);
	}
	it = 0;
L10:
	++it;
	...
	doublereal *hg)
{
    ...
    static doublereal y, a0, a1, r1, r2, x0, y0, y1;
    static integer la;
    static doublereal pi;
    static integer nl;
...
L25:
	    ...
	    gm = 0.;
	    d__1 = *c__ - *a - *b;
	    mcab = (integer) (*c__ - *a - *b + eps * d_sign(&c_b530, &d__1));
...
L105:
	...
	d__1 = *a - *b;
	mab = (integer) (*a - *b + eps * d_sign(&c_b530, &d__1));
...
L115:
		...
		*b = aa;
	    ...
	    ca = *c__ - *a;
	    cb = *c__ - *b;
	    nca = (integer) (ca + eps * d_sign(&c_b530, &ca));
	    ncb = (integer) (cb + eps * d_sign(&c_b530, &cb));
...
/*       ============================================================== */
	...
	if (*kf == 2) {
	    d__1 = (doublereal) nr;
	    psq = pow_dd(&d__1, &c_b56) * 2.;
	}
	px = psq - log(pi * psq) / (pi * pi * pow_dd(&psq, &c_b155));
...
L55:
    *r1f *= a0;
    b0 = *kd * *m / pow_dd(x, &c_b155) / (1.f - *kd / (*x * *x)) * *r1f;
    sud = 0.;
    sw = 0.;
    i__1 = nm;
    for (k = 1; k <= i__1; ++k) {
...
L65:
	r__ = *x * -.25 * x2;
	gs = 1.5;
	*her = r__ * 1.5 - *ber / *x - (log(*x / 2.) + el) * *der + pi * .25 *
		 *dei;
	for (m = 1; m <= 60; ++m) {
/* Computing 2nd power */
	    d__1 = m * 2. + 1.;
...
L75:
	r__ = *x * .5;
	gs = 1.;
	*hei = *x * .5 - *bei / *x - (log(*x / 2.) + el) * *dei - pi * .25 * *
		der;
	for (m = 1; m <= 60; ++m) {
...
/*          https://dlmf.nist.gov/10.21#E40 */
	d__1 = (doublereal) (*n);
	d__2 = (doublereal) (*n);
	x = *n + pow_dd(&d__1, &c_b1174) * 1.85576f + 1.03315f / pow_dd(&d__2,
		 &c_b1174);
    }
/*       iterate */
    l = 0;
...
/*             coefficients from Olver 1951 */
	    d__1 = (doublereal) (*n);
	    d__2 = (doublereal) (*n);
	    x = *n + pow_dd(&d__1, &c_b1174) * 3.2446f + 3.15824f / pow_dd(&
		    d__2, &c_b1174);
	}
    } else {
/*          growth of roots is approximately linear (https://dlmf.nist.gov/10.21#E19) */
    ...
    if (l <= *n + 10) {
	jyndd_(n, &x, &bjn, &djn, &fjn, &byn, &dyn, &fyn);
	h__ = atan(abs(djn) / sqrt((d__1 = fjn * bjn, abs(d__1))));
	b = -djn / (bjn * tan(h__));
	x -= (h__ - pi / 2) / b;
    }
    if (l < *nt) {
...
/*          https://dlmf.nist.gov/10.21#E40 */
	d__1 = (doublereal) (*n);
	d__2 = (doublereal) (*n);
	x = *n + pow_dd(&d__1, &c_b1174) * .80861f + .07249f / pow_dd(&d__2, &
		c_b1174);
    }
/*       iterate */
    l = 0;
...
/*          https://dlmf.nist.gov/10.21#E40 */
	d__1 = (doublereal) (*n);
	d__2 = (doublereal) (*n);
	x = *n + pow_dd(&d__1, &c_b1174) * .93158f + .26035f / pow_dd(&d__2, &
		c_b1174);
    }
/*       iterate */
    l = 0;
...
/*       initial guess for y_{N,L+1} */
    if (l == 1) {
	if (*n == 0) {
	    x = 3.957678419314858f;
	} else {
/*             Expansion from https://dlmf.nist.gov/10.21#E33 and */
/*             coefficients from Olver 1951 */
	    d__1 = (doublereal) (*n);
	    d__2 = (doublereal) (*n);
	    x = *n + pow_dd(&d__1, &c_b1174) * 2.59626f + 2.022183f / pow_dd(&
		    d__2, &c_b1174);
	}
    } else {
/*          growth of roots is approximately linear (https://dlmf.nist.gov/10.21#E19) */
    ...
    if (l <= *n + 10) {
	jyndd_(n, &x, &bjn, &djn, &fjn, &byn, &dyn, &fyn);
	h__ = atan(abs(dyn) / sqrt((d__1 = fyn * byn, abs(d__1))));
	b = -dyn / (byn * tan(h__));
	x -= (h__ - pi / 2) / b;
    }
    if (l < *nt) {
...
/*          https://dlmf.nist.gov/10.21#E40 */
	d__1 = (doublereal) (*n);
	d__2 = (doublereal) (*n);
	x = *n + pow_dd(&d__1, &c_b1174) * 1.8211f + .94001f / pow_dd(&d__2, &
		c_b1174);
    }
/*       iterate */
    l = 0;
...
/*       ======================================================== */
    ...
    hn = 1. / *n;
    d__1 = (doublereal) (*n);
    zl = pow_dd(&d__1, &c_b56) * 1.46 - 1.1611;
    z__ = 0.;
    hf = 0.;
    hd = 0.;
    i__1 = *n / 2;
