	...
	doublereal *workd)
{
    ...
    extern /* Subroutine */ int dscal_(integer *, doublereal *, doublereal *, 
	    ...
	    *, doublereal *, integer *), daxpy_(integer *, doublereal *, 
	    doublereal *, integer *, doublereal *, integer *), dvout_(integer 
	    *, integer *, doublereal *, integer *, char *, ftnlen), ivout_(
	    integer *, integer *, integer *, integer *, char *, ftnlen);
    extern doublereal dlamch_(char *);
    extern /* Subroutine */ int arscnd_(real *), dlacpy_(char *, integer *, 
...
/*        %------------------------------------------------% */
	...
	for (i__ = istart; i__ <= i__2; ++i__) {
		...
		if (msglvl > 0) {
		    ...
		    dvout_(&logfil, &c__1, &h__[i__ + 1 + h_dim1], &ndigit, 
...
/*     %------------------------------------------% */
	...
	if (h__[i__ + 1 + h_dim1] <= epsmch * big) {
		...
		dvout_(&logfil, &c__1, &h__[i__ + 1 + h_dim1], &ndigit, "_sa"
...
/*     %-------------------------------------% */
    ...
    if (msglvl > 1) {
	dvout_(&logfil, &c__1, &q[kplusp + *kev * q_dim1], &ndigit, "_sapps:"
		" sigmak of the updated residual vector", (ftnlen)45);
	dvout_(&logfil, &c__1, &h__[*kev + 1 + h_dim1], &ndigit, "_sapps: be"
		"tak of the updated residual vector", (ftnlen)44);
	dvout_(&logfil, kev, &h__[(h_dim1 << 1) + 1], &ndigit, "_sapps: upda"
		"ted main diagonal of H for next iteration", (ftnlen)53);
	if (*kev > 1) {
	    i__1 = *kev - 1;
	    dvout_(&logfil, &i__1, &h__[h_dim1 + 2], &ndigit, "_sapps: updat"
