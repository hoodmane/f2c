	...
	resid, complex *q, integer *ldq, complex *workl, complex *workd)
{
    ...
    extern /* Subroutine */ int cgemv_(char *, integer *, integer *, complex *
	    ...
	    , integer *, complex *, integer *, char *, ftnlen), ivout_(
    ...
    extern /* Subroutine */ int arscnd_(real *), clacpy_(char *, integer *, 
	    integer *, complex *, integer *, complex *, integer *), clartg_(
	    complex *, complex *, real *, complex *, complex *), claset_(char 
	    *, integer *, integer *, complex *, complex *, complex *, integer 
	    *);
    static integer logfil, ndigit, mcapps;
    static real tcapps;
    static integer msglvl, istart;
    static real smlnum;
...
/*     %----------------------------------------------% */
	...
	if (msglvl > 2) {
	    ivout_(&logfil, &c__1, &jj, &ndigit, "_napps: shift number.", (
		    ftnlen)21);
	    cvout_(&logfil, &c__1, &sigma, &ndigit, "_napps: Value of the sh"
		    "ift ", (ftnlen)27);
	}
...
/*           %----------------------------------------% */
	    ...
	    tst1 = (r__1 = h__[i__3].r, abs(r__1)) + (r__2 = r_imag(&h__[i__ 
		    + i__ * h_dim1]), abs(r__2)) + ((r__3 = h__[i__4].r, abs(
		    r__3)) + (r__4 = r_imag(&h__[i__ + 1 + (i__ + 1) * h_dim1]
		    ), abs(r__4)));
	    if (tst1 == 0.f) {
		i__3 = kplusp - jj + 1;
...
/* Computing MAX */
	    r__2 = ulp * tst1;
	    if ((r__1 = h__[i__3].r, abs(r__1)) <= max(r__2,smlnum)) {
		if (msglvl > 0) {
		    ...
		    cvout_(&logfil, &c__1, &h__[i__ + 1 + i__ * h_dim1], &
...
/*        %--------------------------------------------% */
	...
	i__2 = i__ + i__ * h_dim1;
	i__3 = i__ + 1 + (i__ + 1) * h_dim1;
	tst1 = (r__1 = h__[i__2].r, abs(r__1)) + (r__2 = r_imag(&h__[i__ + 
		i__ * h_dim1]), abs(r__2)) + ((r__3 = h__[i__3].r, abs(r__3)) 
		+ (r__4 = r_imag(&h__[i__ + 1 + (i__ + 1) * h_dim1]), abs(
		r__4)));
	if (tst1 == 0.f) {
...
/*     %-------------------------------------% */
    ...
    if (msglvl > 1) {
	cvout_(&logfil, &c__1, &q[kplusp + *kev * q_dim1], &ndigit, "_napps:"
		" sigmak = (e_{kev+p}^T*Q)*e_{kev}", (ftnlen)40);
	cvout_(&logfil, &c__1, &h__[*kev + 1 + *kev * h_dim1], &ndigit, "_na"
	...
	if (msglvl > 2) {
	    cmout_(&logfil, kev, kev, &h__[h_offset], ldh, &ndigit, "_napps:"
