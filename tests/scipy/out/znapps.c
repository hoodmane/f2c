	...
	ldq, doublecomplex *workl, doublecomplex *workd)
{
    ...
    extern /* Subroutine */ int zscal_(integer *, doublecomplex *, 
	    ...
	    integer *, doublecomplex *, doublecomplex *, integer *), zcopy_(
	    integer *, doublecomplex *, integer *, doublecomplex *, integer *)
	    , ivout_(integer *, integer *, integer *, integer *, char *, 
	    ftnlen), zaxpy_(integer *, doublecomplex *, doublecomplex *, 
	    integer *, doublecomplex *, integer *), zmout_(integer *, integer 
    ...
    extern /* Subroutine */ int zlacpy_(char *, integer *, integer *, 
	    doublecomplex *, integer *, doublecomplex *, integer *), zlartg_(
...
/*     %----------------------------------------------% */
	...
	if (msglvl > 2) {
	    ivout_(&logfil, &c__1, &jj, &ndigit, "_napps: shift number.", (
		    ftnlen)21);
	    zvout_(&logfil, &c__1, &sigma, &ndigit, "_napps: Value of the sh"
		    "ift ", (ftnlen)27);
	}
...
/* Computing MAX */
		...
		if (msglvl > 0) {
		    ...
		    zvout_(&logfil, &c__1, &h__[i__ + 1 + i__ * h_dim1], &
...
/*     %-------------------------------------% */
    ...
    if (msglvl > 1) {
	zvout_(&logfil, &c__1, &q[kplusp + *kev * q_dim1], &ndigit, "_napps:"
		" sigmak = (e_{kev+p}^T*Q)*e_{kev}", (ftnlen)40);
	zvout_(&logfil, &c__1, &h__[*kev + 1 + *kev * h_dim1], &ndigit, "_na"
	...
	if (msglvl > 2) {
	    zmout_(&logfil, kev, kev, &h__[h_offset], ldh, &ndigit, "_napps:"
