...
/*     wcdotc   Level 1 BLAS that computes the scalar product of two vectors. */
	...
	which_len)
{
    ...
    extern /* Subroutine */ int ivout_(integer *, integer *, integer *, 
	    ...
	    integer *, char *, ftnlen);
    static integer mcaup2;
    extern /* Subroutine */ int cgetv0_(integer *, char *, integer *, logical 
    ...
    extern /* Subroutine */ int cngets_(integer *, char *, integer *, integer 
	    *, complex *, complex *, ftnlen), cnapps_(integer *, integer *, 
...
/* L25: */
	...
	kp[1] = *np;
	kp[2] = nconv;
	ivout_(&logfil, &c__3, kp, &ndigit, "_naup2: NEV, NP, NCONV are", (
		ftnlen)26);
	cvout_(&logfil, &kplusp, &ritz[1], &ndigit, "_naup2: The eigenvalues"
		" of H", (ftnlen)28);
	cvout_(&logfil, &kplusp, &bounds[1], &ndigit, "_naup2: Ritz estimate"
...
/* Computing 2nd power */
	    i__1 = kplusp;
	    cvout_(&logfil, &kplusp, &workl[i__1 * i__1 + 1], &ndigit, "_nau"
		    "p2: Eigenvalues computed by _neigh:", (ftnlen)39);
/* Computing 2nd power */
	    i__1 = kplusp;
	    cvout_(&logfil, &kplusp, &workl[i__1 * i__1 + kplusp + 1], &
		    ndigit, "_naup2: Ritz estimates computed by _neigh:", (
		    ftnlen)42);
	}
...
/*           %-----------------------------------------------% */
	...
	if (msglvl > 1) {
	    cvout_(&logfil, &kplusp, &ritz[1], &ndigit, "_naup2: Sorted eige"
		    "nvalues", (ftnlen)26);
	    cvout_(&logfil, &kplusp, &bounds[1], &ndigit, "_naup2: Sorted ri"
...
/*           %---------------------------------------% */
	...
	if (msglvl > 1) {
	    ...
	    ivout_(&logfil, &c__2, kp, &ndigit, "_naup2: NEV and NP are", (
		    ftnlen)22);
	    cvout_(&logfil, nev, &ritz[*np + 1], &ndigit, "_naup2: \"wante"
		    "d\" Ritz values ", (ftnlen)29);
	    cvout_(&logfil, nev, &bounds[*np + 1], &ndigit, "_naup2: Ritz es"
...
/*            %----------------------------------% */
	...
	ivout_(&logfil, &c__1, np, &ndigit, "_naup2: The number of shifts to"
		" apply ", (ftnlen)38);
	cvout_(&logfil, np, &ritz[1], &ndigit, "_naup2: values of the shifts",
		 (ftnlen)28);
	if (*ishift == 1) {
	    cvout_(&logfil, np, &bounds[1], &ndigit, "_naup2: Ritz estimates"
...
/*        %----------------------------------% */
    ...
    if (*(unsigned char *)bmat == 'G') {
	wcdotc_(&q__1, n, &resid[1], &c__1, &workd[1], &c__1);
	...
	cmout_(&logfil, nev, nev, &h__[h_offset], ldh, &ndigit, "_naup2: Com"
