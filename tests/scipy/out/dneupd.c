	...
	ftnlen which_len)
{
    ...
    extern /* Subroutine */ int dtrmm_(char *, char *, char *, char *, 
	    ...
	    doublereal *, integer *), dmout_(integer *, integer *, integer *, 
    ...
    extern /* Subroutine */ int dlahqr_(logical *, logical *, integer *, 
	    ...
	    integer *, doublereal *, integer *), dlaset_(char *, integer *, 
    ...
    static integer msglvl, invsub, numcnv, iuptri, outncv;
...
/*     %------------------------------------% */
    ...
    if (msglvl > 2) {
	dvout_(&logfil, ncv, &workl[irr], &ndigit, "_neupd: Real part of Rit"
		"z values passed in from _NAUPD.", (ftnlen)55);
	dvout_(&logfil, ncv, &workl[iri], &ndigit, "_neupd: Imag part of Rit"
		"z values passed in from _NAUPD.", (ftnlen)55);
	dvout_(&logfil, ncv, &workl[ibd], &ndigit, "_neupd: Ritz estimates p"
...
/*        %-------------------------------------% */
	...
	if (msglvl > 2) {
	    dvout_(&logfil, ncv, &workl[irr], &ndigit, "_neupd: Real part of"
		    " Ritz values after calling _NGETS.", (ftnlen)54);
	    dvout_(&logfil, ncv, &workl[iri], &ndigit, "_neupd: Imag part of"
		    " Ritz values after calling _NGETS.", (ftnlen)54);
	    dvout_(&logfil, ncv, &workl[bounds], &ndigit, "_neupd: Ritz valu"
...
/*        %-----------------------------------------------------------% */
	...
	if (msglvl > 1) {
	    dvout_(&logfil, ncv, &workl[iheigr], &ndigit, "_neupd: Real part"
		    " of the eigenvalues of H", (ftnlen)41);
	    dvout_(&logfil, ncv, &workl[iheigi], &ndigit, "_neupd: Imaginary"
		    " part of the Eigenvalues of H", (ftnlen)46);
	    dvout_(&logfil, ncv, &workl[ihbds], &ndigit, "_neupd: Last row o"
		    "f the Schur vector matrix", (ftnlen)43);
	    if (msglvl > 3) {
		dmout_(&logfil, ncv, ncv, &workl[iuptri], &ldh, &ndigit, 
...
/*           %-----------------------------------------------------% */
		...
		*info = 1;
	    ...
	    if (msglvl > 2) {
		dvout_(&logfil, ncv, &workl[iheigr], &ndigit, "_neupd: Real "
			"part of the eigenvalues of H--reordered", (ftnlen)52);
		dvout_(&logfil, ncv, &workl[iheigi], &ndigit, "_neupd: Imag "
			"part of the eigenvalues of H--reordered", (ftnlen)52);
		if (msglvl > 3) {
		    dmout_(&logfil, ncv, ncv, &workl[iuptri], &ldq, &ndigit, 
...
/* L45: */
		...
		dcopy_(ncv, &workl[invsub + *ncv - 1], &ldq, &workl[ihbds], &
			c__1);
		dvout_(&logfil, ncv, &workl[ihbds], &ndigit, "_neupd: Last r"
			"ow of the eigenvector matrix for T", (ftnlen)48);
		if (msglvl > 3) {
		    dmout_(&logfil, ncv, ncv, &workl[invsub], &ldq, &ndigit, 
...
/* L80: */
	...
	dvout_(&logfil, &nconv, &dr[1], &ndigit, "_neupd: Untransformed real"
		" part of the Ritz valuess.", (ftnlen)52);
	dvout_(&logfil, &nconv, &di[1], &ndigit, "_neupd: Untransformed imag"
		" part of the Ritz valuess.", (ftnlen)52);
	dvout_(&logfil, &nconv, &workl[ihbds], &ndigit, "_neupd: Ritz estima"
		"tes of untransformed Ritz values.", (ftnlen)52);
    } else if (s_cmp(type__, "REGULR", (ftnlen)6, (ftnlen)6) == 0 && msglvl > 
	    1) {
	dvout_(&logfil, &nconv, &dr[1], &ndigit, "_neupd: Real parts of conv"
		"erged Ritz values.", (ftnlen)44);
	dvout_(&logfil, &nconv, &di[1], &ndigit, "_neupd: Imag parts of conv"
		"erged Ritz values.", (ftnlen)44);
	dvout_(&logfil, &nconv, &workl[ihbds], &ndigit, "_neupd: Associated "
