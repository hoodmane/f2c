	...
	which_len)
{
    ...
    extern /* Subroutine */ int svout_(integer *, integer *, real *, integer *
	    , char *, ftnlen), snaup2_(integer *, char *, integer *, char *, 
    ...
    static real tnaupd, titref, tnaitr;
...
/*     %------------------------------------% */
	...
	*info = 3;
	...
	svout_(&logfil, &np, &workl[ritzr], &ndigit, "_naupd: Real part of t"
		"he final Ritz values", (ftnlen)42);
	svout_(&logfil, &np, &workl[ritzi], &ndigit, "_naupd: Imaginary part"
		" of the final Ritz values", (ftnlen)47);
	svout_(&logfil, &np, &workl[bounds], &ndigit, "_naupd: Associated Ri"
