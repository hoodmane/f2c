...
/*     %-------------------------------% */
    ...
    if (msglvl > 0) {
	svout_(&logfil, n, &h__[(h_dim1 << 1) + 1], &ndigit, "_seigt: main d"
		"iagonal of matrix H", (ftnlen)33);
	if (*n > 1) {
	    i__1 = *n - 1;
	    svout_(&logfil, &i__1, &h__[h_dim1 + 2], &ndigit, "_seigt: sub d"
    ...
    if (msglvl > 1) {
	svout_(&logfil, n, &bounds[1], &ndigit, "_seigt: last row of the eig"
...
/*     %-----------------------------------------------% */
    ...
    i__1 = *n;
    for (k = 1; k <= i__1; ++k) {
	bounds[k] = *rnorm * (r__1 = bounds[k], abs(r__1));
/* L30: */
    }
