	...
	workd, integer *info, ftnlen bmat_len, ftnlen which_len)
{
    ...
    extern /* Subroutine */ int sgetv0_(integer *, char *, integer *, logical 
    ...
    extern /* Subroutine */ int sneigh_(real *, integer *, real *, integer *, 
    ...
    static integer msglvl, nptemp, numcnv;
    extern /* Subroutine */ int snconv_(integer *, real *, real *, real *, 
	    real *, integer *), sngets_(integer *, char *, integer *, integer 
	    *, real *, real *, real *, real *, real *, ftnlen), snapps_(
	    integer *, integer *, integer *, real *, real *, real *, integer *
	    , real *, integer *, real *, real *, integer *, real *, real *), 
	    ssortc_(char *, logical *, integer *, real *, real *, real *, 
	    ftnlen);
...
L1000:
    ...
    if (msglvl > 0) {
	ivout_(&logfil, &c__1, &iter, &ndigit, "_naup2: **** Start of major "
...
/*        %-----------------------------------------------------------% */
    ...
    *np = kplusp - *nev;
    ...
    if (msglvl > 1) {
	ivout_(&logfil, &c__1, nev, &ndigit, "_naup2: The length of the curr"
		"ent Arnoldi factorization", (ftnlen)55);
	ivout_(&logfil, &c__1, np, &ndigit, "_naup2: Extend the Arnoldi fact"
...
/*        %---------------------------------------------------% */
	...
	*info = -9999;
    ...
    if (msglvl > 1) {
	svout_(&logfil, &c__1, &rnorm, &ndigit, "_naup2: Corresponding B-nor"
...
/*        %-------------------% */
	...
	kp[2] = numcnv;
	kp[3] = nconv;
	ivout_(&logfil, &c__4, kp, &ndigit, "_naup2: NEV, NP, NUMCNV, NCONV "
		"are", (ftnlen)34);
	svout_(&logfil, &kplusp, &ritzr[1], &ndigit, "_naup2: Real part of t"
		"he eigenvalues of H", (ftnlen)41);
	svout_(&logfil, &kplusp, &ritzi[1], &ndigit, "_naup2: Imaginary part"
		" of the eigenvalues of H", (ftnlen)46);
	svout_(&logfil, &kplusp, &bounds[1], &ndigit, "_naup2: Ritz estimate"
...
/* Computing 2nd power */
	    i__1 = kplusp;
	    svout_(&logfil, &kplusp, &workl[i__1 * i__1 + 1], &ndigit, "_nau"
		    "p2: Real part of the eig computed by _neigh:", (ftnlen)48)
		    ;
/* Computing 2nd power */
	    i__1 = kplusp;
	    svout_(&logfil, &kplusp, &workl[i__1 * i__1 + kplusp + 1], &
		    ndigit, "_naup2: Imag part of the eig computed by _neigh:"
		    , (ftnlen)48);
/* Computing 2nd power */
	    i__1 = kplusp;
	    svout_(&logfil, &kplusp, &workl[i__1 * i__1 + (kplusp << 1) + 1], 
...
/*           %------------------------------------------------% */
	...
	if (msglvl > 1) {
	    svout_(&logfil, &kplusp, &ritzr[1], &ndigit, "_naup2: Sorted rea"
		    "l part of the eigenvalues", (ftnlen)43);
	    svout_(&logfil, &kplusp, &ritzi[1], &ndigit, "_naup2: Sorted ima"
		    "ginary part of the eigenvalues", (ftnlen)48);
	    svout_(&logfil, &kplusp, &bounds[1], &ndigit, "_naup2: Sorted ri"
...
/*           %---------------------------------------% */
    ...
    if (msglvl > 0) {
	ivout_(&logfil, &c__1, &nconv, &ndigit, "_naup2: no. of \"converge"
		"d\" Ritz values at this iter.", (ftnlen)52);
	if (msglvl > 1) {
	    kp[0] = *nev;
	    kp[1] = *np;
	    ivout_(&logfil, &c__2, kp, &ndigit, "_naup2: NEV and NP are", (
		    ftnlen)22);
	    svout_(&logfil, nev, &ritzr[*np + 1], &ndigit, "_naup2: \"wante"
		    "d\" Ritz values -- real part", (ftnlen)41);
	    svout_(&logfil, nev, &ritzi[*np + 1], &ndigit, "_naup2: \"wante"
		    "d\" Ritz values -- imag part", (ftnlen)41);
	    svout_(&logfil, nev, &bounds[*np + 1], &ndigit, "_naup2: Ritz es"
...
/*            %----------------------------------% */
    ...
    if (msglvl > 2) {
	ivout_(&logfil, &c__1, np, &ndigit, "_naup2: The number of shifts to"
		" apply ", (ftnlen)38);
	svout_(&logfil, np, &ritzr[1], &ndigit, "_naup2: Real part of the sh"
		"ifts", (ftnlen)31);
	svout_(&logfil, np, &ritzi[1], &ndigit, "_naup2: Imaginary part of t"
		"he shifts", (ftnlen)36);
	if (*ishift == 1) {
	    svout_(&logfil, np, &bounds[1], &ndigit, "_naup2: Ritz estimates"
...
/*        %----------------------------------% */
    ...
    if (msglvl > 2) {
	svout_(&logfil, &c__1, &rnorm, &ndigit, "_naup2: B-norm of residual "
		"for compressed factorization", (ftnlen)55);
	smout_(&logfil, nev, nev, &h__[h_offset], ldh, &ndigit, "_naup2: Com"
