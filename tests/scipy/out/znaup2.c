...
/*     wzdotc    Level 1 BLAS that computes the scalar product of two vectors. */
	...
	doublereal *rwork, integer *info, ftnlen bmat_len, ftnlen which_len)
{
    ...
    extern /* Subroutine */ int zneigh_(doublereal *, integer *, 
    ...
    extern /* Subroutine */ int znaitr_(integer *, char *, integer *, integer 
	    *, integer *, integer *, doublecomplex *, doublereal *, 
	    doublecomplex *, integer *, doublecomplex *, integer *, integer *,
	     doublecomplex *, integer *, ftnlen), zngets_(integer *, char *, 
	    integer *, integer *, doublecomplex *, doublecomplex *, ftnlen), 
	    znapps_(integer *, integer *, integer *, doublecomplex *, 
...
/* L25: */
	...
	ivout_(&logfil, &c__3, kp, &ndigit, "_naup2: NEV, NP, NCONV are", (
		ftnlen)26);
	zvout_(&logfil, &kplusp, &ritz[1], &ndigit, "_naup2: The eigenvalues"
		" of H", (ftnlen)28);
	zvout_(&logfil, &kplusp, &bounds[1], &ndigit, "_naup2: Ritz estimate"
...
/* Computing 2nd power */
	    i__1 = kplusp;
	    zvout_(&logfil, &kplusp, &workl[i__1 * i__1 + 1], &ndigit, "_nau"
		    "p2: Eigenvalues computed by _neigh:", (ftnlen)39);
/* Computing 2nd power */
	    i__1 = kplusp;
	    zvout_(&logfil, &kplusp, &workl[i__1 * i__1 + kplusp + 1], &
...
/*           %-----------------------------------------------% */
	...
	if (msglvl > 1) {
	    zvout_(&logfil, &kplusp, &ritz[1], &ndigit, "_naup2: Sorted eige"
		    "nvalues", (ftnlen)26);
	    zvout_(&logfil, &kplusp, &bounds[1], &ndigit, "_naup2: Sorted ri"
...
/*           %---------------------------------------% */
	...
	if (msglvl > 1) {
	    ...
	    ivout_(&logfil, &c__2, kp, &ndigit, "_naup2: NEV and NP are", (
		    ftnlen)22);
	    zvout_(&logfil, nev, &ritz[*np + 1], &ndigit, "_naup2: \"wante"
		    "d\" Ritz values ", (ftnlen)29);
	    zvout_(&logfil, nev, &bounds[*np + 1], &ndigit, "_naup2: Ritz es"
...
/*            %----------------------------------% */
	...
	ivout_(&logfil, &c__1, np, &ndigit, "_naup2: The number of shifts to"
		" apply ", (ftnlen)38);
	zvout_(&logfil, np, &ritz[1], &ndigit, "_naup2: values of the shifts",
		 (ftnlen)28);
	if (*ishift == 1) {
	    zvout_(&logfil, np, &bounds[1], &ndigit, "_naup2: Ritz estimates"
...
/*        %----------------------------------% */
    ...
    if (*(unsigned char *)bmat == 'G') {
	wzdotc_(&z__1, n, &resid[1], &c__1, &workd[1], &c__1);
	...
	zmout_(&logfil, nev, nev, &h__[h_offset], ldh, &ndigit, "_naup2: Com"
