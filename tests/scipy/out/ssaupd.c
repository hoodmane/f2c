	...
	which_len)
{
    ...
    extern /* Subroutine */ int ivout_(integer *, integer *, integer *, 
	    ...
	    integer *, char *, ftnlen);
    static real tgetv0;
    extern /* Subroutine */ int ssaup2_(integer *, char *, integer *, char *, 
    ...
    static real titref, tsaupd, tseigt;
...
/*     %------------------------------------% */
	...
	*info = 3;
    ...
    if (msglvl > 0) {
	ivout_(&logfil, &c__1, &mxiter, &ndigit, "_saupd: number of update i"
		"terations taken", (ftnlen)41);
	ivout_(&logfil, &c__1, &np, &ndigit, "_saupd: number of \"converge"
		"d\" Ritz values", (ftnlen)41);
	svout_(&logfil, &np, &workl[ritz], &ndigit, "_saupd: final Ritz valu"
		"es", (ftnlen)25);
	svout_(&logfil, &np, &workl[bounds], &ndigit, "_saupd: corresponding"
