...
L80:
	...
	xtemp[0] = wnorm;
	xtemp[1] = *rnorm;
	svout_(&logfil, &c__2, xtemp, &ndigit, "_naitr: re-orthonalization; "
		"wnorm and rnorm are", (ftnlen)47);
	svout_(&logfil, &j, &h__[j * h_dim1 + 1], &ndigit, "_naitr: j-th col"
		"umn of H", (ftnlen)24);
    }
...
/*        %-----------------------------------------------------% */
	...
	if (msglvl > 2) {
	    xtemp[0] = *rnorm;
	    xtemp[1] = rnorm1;
	    svout_(&logfil, &c__2, xtemp, &ndigit, "_naitr: iterative refine"
...
/* L110: */
	...
	if (msglvl > 2) {
	    i__1 = *k + *np;
	    i__2 = *k + *np;
	    smout_(&logfil, &i__1, &i__2, &h__[h_offset], ldh, &ndigit, "_na"
