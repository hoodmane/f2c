	...
	ftnlen which_len)
{
    ...
    static integer logfil, ndigit, ishift;
    extern /* Subroutine */ int dsgets_(integer *, char *, integer *, integer 
	    *, doublereal *, doublereal *, doublereal *, ftnlen);
    static integer bounds, mseupd;
    extern /* Subroutine */ int dsteqr_(char *, integer *, doublereal *, 
	    doublereal *, doublereal *, integer *, doublereal *, integer *);
    static integer msglvl;
    extern /* Subroutine */ int dsesrt_(char *, logical *, integer *, 
...
/*     %---------------------------------------% */
    ...
    if (msglvl > 2) {
	dvout_(&logfil, ncv, &workl[irz], &ndigit, "_seupd: Ritz values pass"
		"ed in from _SAUPD.", (ftnlen)42);
	dvout_(&logfil, ncv, &workl[ibd], &ndigit, "_seupd: Ritz estimates p"
...
/*        %-------------------------------------% */
	...
	if (msglvl > 2) {
	    dvout_(&logfil, ncv, &workl[irz], &ndigit, "_seupd: Ritz values "
		    "after calling _SGETS.", (ftnlen)41);
	    dvout_(&logfil, ncv, &workl[bounds], &ndigit, "_seupd: Ritz valu"
...
/*        %-----------------------------------------------------------% */
	...
	if (msglvl > 1) {
	    ...
	    dvout_(&logfil, ncv, &workl[ihd], &ndigit, "_seupd: NCV Ritz val"
		    "ues of the final H matrix", (ftnlen)45);
	    dvout_(&logfil, ncv, &workl[iw], &ndigit, "_seupd: last row of t"
...
L30:
	if (msglvl > 2) {
	    dvout_(&logfil, ncv, &workl[ihd], &ndigit, "_seupd: The eigenval"
...
/* L100: */
	...
	dvout_(&logfil, &nconv, &d__[1], &ndigit, "_seupd: Untransformed con"
		"verged Ritz values", (ftnlen)43);
	dvout_(&logfil, &nconv, &workl[ihb], &ndigit, "_seupd: Ritz estimate"
		"s of the untransformed Ritz values", (ftnlen)55);
    } else if (msglvl > 1) {
	dvout_(&logfil, &nconv, &d__[1], &ndigit, "_seupd: Converged Ritz va"
		"lues", (ftnlen)29);
	dvout_(&logfil, &nconv, &workl[ihb], &ndigit, "_seupd: Associated Ri"
