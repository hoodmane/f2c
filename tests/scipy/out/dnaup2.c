	 ...
	 integer *info, ftnlen bmat_len, ftnlen which_len)
{
    ...
    extern /* Subroutine */ int dngets_(integer *, char *, integer *, integer 
	    *, doublereal *, doublereal *, doublereal *, doublereal *, 
	    doublereal *, ftnlen), dnapps_(integer *, integer *, integer *, 
	    doublereal *, doublereal *, doublereal *, integer *, doublereal *,
	     integer *, doublereal *, doublereal *, integer *, doublereal *, 
	    doublereal *), dnconv_(integer *, doublereal *, doublereal *, 
	    doublereal *, doublereal *, integer *), dsortc_(char *, logical *,
...
/*        %---------------------------------------------------% */
	...
	*info = -9999;
    ...
    if (msglvl > 1) {
	dvout_(&logfil, &c__1, &rnorm, &ndigit, "_naup2: Corresponding B-nor"
...
/*        %--------------------------------------------------------% */
    ...
    dneigh_(&rnorm, &kplusp, &h__[h_offset], ldh, &ritzr[1], &ritzi[1], &
...
/*        %-------------------% */
	...
	ivout_(&logfil, &c__4, kp, &ndigit, "_naup2: NEV, NP, NUMCNV, NCONV "
		"are", (ftnlen)34);
	dvout_(&logfil, &kplusp, &ritzr[1], &ndigit, "_naup2: Real part of t"
		"he eigenvalues of H", (ftnlen)41);
	dvout_(&logfil, &kplusp, &ritzi[1], &ndigit, "_naup2: Imaginary part"
		" of the eigenvalues of H", (ftnlen)46);
	dvout_(&logfil, &kplusp, &bounds[1], &ndigit, "_naup2: Ritz estimate"
...
/* Computing 2nd power */
	    i__1 = kplusp;
	    dvout_(&logfil, &kplusp, &workl[i__1 * i__1 + 1], &ndigit, "_nau"
		    "p2: Real part of the eig computed by _neigh:", (ftnlen)48)
		    ;
/* Computing 2nd power */
	    i__1 = kplusp;
	    dvout_(&logfil, &kplusp, &workl[i__1 * i__1 + kplusp + 1], &
		    ndigit, "_naup2: Imag part of the eig computed by _neigh:"
		    , (ftnlen)48);
/* Computing 2nd power */
	    i__1 = kplusp;
	    dvout_(&logfil, &kplusp, &workl[i__1 * i__1 + (kplusp << 1) + 1], 
...
/*           %------------------------------------------------% */
	...
	if (msglvl > 1) {
	    dvout_(&logfil, &kplusp, &ritzr[1], &ndigit, "_naup2: Sorted rea"
		    "l part of the eigenvalues", (ftnlen)43);
	    dvout_(&logfil, &kplusp, &ritzi[1], &ndigit, "_naup2: Sorted ima"
		    "ginary part of the eigenvalues", (ftnlen)48);
	    dvout_(&logfil, &kplusp, &bounds[1], &ndigit, "_naup2: Sorted ri"
...
/*           %---------------------------------------% */
	...
	if (msglvl > 1) {
	    ...
	    ivout_(&logfil, &c__2, kp, &ndigit, "_naup2: NEV and NP are", (
		    ftnlen)22);
	    dvout_(&logfil, nev, &ritzr[*np + 1], &ndigit, "_naup2: \"wante"
		    "d\" Ritz values -- real part", (ftnlen)41);
	    dvout_(&logfil, nev, &ritzi[*np + 1], &ndigit, "_naup2: \"wante"
		    "d\" Ritz values -- imag part", (ftnlen)41);
	    dvout_(&logfil, nev, &bounds[*np + 1], &ndigit, "_naup2: Ritz es"
...
/*            %----------------------------------% */
	...
	ivout_(&logfil, &c__1, np, &ndigit, "_naup2: The number of shifts to"
		" apply ", (ftnlen)38);
	dvout_(&logfil, np, &ritzr[1], &ndigit, "_naup2: Real part of the sh"
		"ifts", (ftnlen)31);
	dvout_(&logfil, np, &ritzi[1], &ndigit, "_naup2: Imaginary part of t"
		"he shifts", (ftnlen)36);
	if (*ishift == 1) {
	    dvout_(&logfil, np, &bounds[1], &ndigit, "_naup2: Ritz estimates"
...
/*        %---------------------------------------------------------% */
    ...
    dnapps_(n, nev, np, &ritzr[1], &ritzi[1], &v[v_offset], ldv, &h__[
...
/*        %----------------------------------% */
    ...
    if (msglvl > 2) {
	dvout_(&logfil, &c__1, &rnorm, &ndigit, "_naup2: B-norm of residual "
		"for compressed factorization", (ftnlen)55);
	dmout_(&logfil, nev, nev, &h__[h_offset], ldh, &ndigit, "_naup2: Com"
