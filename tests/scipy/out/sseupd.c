	...
	ftnlen bmat_len, ftnlen which_len)
{
    ...
    static integer logfil, ndigit, ishift, bounds;
    extern /* Subroutine */ int slacpy_(char *, integer *, integer *, real *, 
	    integer *, real *, integer *);
    static integer mseupd, msglvl, numcnv;
    extern /* Subroutine */ int ssgets_(integer *, char *, integer *, integer 
	    *, real *, real *, real *, ftnlen), ssteqr_(char *, integer *, 
	    real *, real *, real *, integer *, real *, integer *), ssesrt_(
	    char *, logical *, integer *, real *, integer *, real *, integer *
	    , ftnlen), ssortr_(char *, logical *, integer *, real *, real *, 
	    ftnlen);
...
/*     %---------------------------------------% */
    ...
    if (msglvl > 2) {
	svout_(&logfil, ncv, &workl[irz], &ndigit, "_seupd: Ritz values pass"
		"ed in from _SAUPD.", (ftnlen)42);
	svout_(&logfil, ncv, &workl[ibd], &ndigit, "_seupd: Ritz estimates p"
...
/*        %-------------------------------------% */
	...
	if (msglvl > 2) {
	    svout_(&logfil, ncv, &workl[irz], &ndigit, "_seupd: Ritz values "
		    "after calling _SGETS.", (ftnlen)41);
	    svout_(&logfil, ncv, &workl[bounds], &ndigit, "_seupd: Ritz valu"
...
/*        %-----------------------------------------------------% */
	...
	for (j = 1; j <= i__1; ++j) {
/* Computing MAX */
	    r__2 = eps23, r__3 = (r__1 = workl[irz + *ncv - j], abs(r__1));
	    temp1 = max(r__2,r__3);
...
/*        %-----------------------------------------------------------% */
	...
	if (msglvl > 2) {
	    ivout_(&logfil, &c__1, &numcnv, &ndigit, "_seupd: Number of spec"
		    "ified eigenvalues", (ftnlen)39);
	    ivout_(&logfil, &c__1, &nconv, &ndigit, "_seupd: Number of \"con"
...
/*        %-----------------------------------------------------------% */
	...
	if (msglvl > 1) {
	    ...
	    svout_(&logfil, ncv, &workl[ihd], &ndigit, "_seupd: NCV Ritz val"
		    "ues of the final H matrix", (ftnlen)45);
	    svout_(&logfil, ncv, &workl[iw], &ndigit, "_seupd: last row of t"
...
L30:
	if (msglvl > 2) {
	    svout_(&logfil, ncv, &workl[ihd], &ndigit, "_seupd: The eigenval"
...
/*     Not yet implemented. See remark 2 above. */
	...
	i__1 = *ncv;
	for (j = 1; j <= i__1; ++j) {
	    workl[ihb + j - 1] = rnorm * (r__1 = workl[ihb + j - 1], abs(r__1)
		    );
/* L70: */
	}
...
/* Computing 2nd power */
		r__2 = workl[iw + k - 1];
		workl[ihb + k - 1] = (r__1 = workl[ihb + k - 1], abs(r__1)) / 
			(r__2 * r__2);
/* L80: */
	    }
...
/* Computing 2nd power */
		...
		workl[ihb + k - 1] = *sigma * (r__1 = workl[ihb + k - 1], abs(
			r__1)) / (r__2 * r__2);
/* L90: */
	    }
...
/* L100: */
	...
	svout_(&logfil, &nconv, &d__[1], &ndigit, "_seupd: Untransformed con"
		"verged Ritz values", (ftnlen)43);
	svout_(&logfil, &nconv, &workl[ihb], &ndigit, "_seupd: Ritz estimate"
		"s of the untransformed Ritz values", (ftnlen)55);
    } else if (msglvl > 1) {
	svout_(&logfil, &nconv, &d__[1], &ndigit, "_seupd: Converged Ritz va"
		"lues", (ftnlen)29);
	svout_(&logfil, &nconv, &workl[ihb], &ndigit, "_seupd: Associated Ri"
