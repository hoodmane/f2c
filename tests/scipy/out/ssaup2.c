	...
	info, ftnlen bmat_len, ftnlen which_len)
{
    ...
    extern /* Subroutine */ int sseigt_(real *, integer *, real *, integer *, 
    ...
    extern /* Subroutine */ int ssaitr_(integer *, char *, integer *, integer 
	    *, integer *, integer *, real *, real *, real *, integer *, real *
	    , integer *, integer *, real *, integer *, ftnlen), ssgets_(
	    integer *, char *, integer *, integer *, real *, real *, real *, 
	    ftnlen), ssapps_(integer *, integer *, integer *, real *, real *, 
	    integer *, real *, integer *, real *, real *, integer *, real *), 
	    ssconv_(integer *, real *, real *, real *, integer *);
...
L1000:
    ...
    if (msglvl > 0) {
	ivout_(&logfil, &c__1, &iter, &ndigit, "_saup2: **** Start of major "
		"iteration number ****", (ftnlen)49);
    }
    if (msglvl > 1) {
	ivout_(&logfil, &c__1, nev, &ndigit, "_saup2: The length of the curr"
		"ent Lanczos factorization", (ftnlen)55);
	ivout_(&logfil, &c__1, np, &ndigit, "_saup2: Extend the Lanczos fact"
...
/*           %-----------------------------------------------------% */
	...
	*info = -9999;
    ...
    if (msglvl > 1) {
	svout_(&logfil, &c__1, &rnorm, &ndigit, "_saup2: Current B-norm of r"
...
/*        %-------------------% */
	...
	kp[1] = *np;
	kp[2] = nconv;
	ivout_(&logfil, &c__3, kp, &ndigit, "_saup2: NEV, NP, NCONV are", (
		ftnlen)26);
	svout_(&logfil, &kplusp, &ritz[1], &ndigit, "_saup2: The eigenvalues"
		" of H", (ftnlen)28);
	svout_(&logfil, &kplusp, &bounds[1], &ndigit, "_saup2: Ritz estimate"
...
/*           %--------------------------------------------------% */
	...
	for (j = 1; j <= i__1; ++j) {
/* Computing MAX */
	    r__2 = eps23, r__3 = (r__1 = ritz[j], abs(r__1));
	    temp = max(r__2,r__3);
	    bounds[j] /= temp;
/* L35: */
	}
...
/*           %------------------------------------------% */
	...
	if (msglvl > 1) {
	    svout_(&logfil, &kplusp, &ritz[1], &ndigit, "_saup2: Sorted Ritz"
		    " values.", (ftnlen)27);
	    svout_(&logfil, &kplusp, &bounds[1], &ndigit, "_saup2: Sorted ri"
...
/*           %---------------------------------------% */
    ...
    if (msglvl > 0) {
	ivout_(&logfil, &c__1, &nconv, &ndigit, "_saup2: no. of \"converge"
		"d\" Ritz values at this iter.", (ftnlen)52);
	if (msglvl > 1) {
	    kp[0] = *nev;
	    kp[1] = *np;
	    ivout_(&logfil, &c__2, kp, &ndigit, "_saup2: NEV and NP are", (
		    ftnlen)22);
	    svout_(&logfil, nev, &ritz[*np + 1], &ndigit, "_saup2: \"wante"
		    "d\" Ritz values.", (ftnlen)29);
	    svout_(&logfil, nev, &bounds[*np + 1], &ndigit, "_saup2: Ritz es"
...
/*        %---------------------------------------------------------% */
    ...
    if (msglvl > 2) {
	ivout_(&logfil, &c__1, np, &ndigit, "_saup2: The number of shifts to"
		" apply ", (ftnlen)38);
	svout_(&logfil, np, &workl[1], &ndigit, "_saup2: shifts selected", (
		ftnlen)23);
	if (*ishift == 1) {
	    svout_(&logfil, np, &bounds[1], &ndigit, "_saup2: corresponding "
...
/* L130: */
    ...
    if (msglvl > 2) {
	svout_(&logfil, &c__1, &rnorm, &ndigit, "_saup2: B-norm of residual "
		"for NEV factorization", (ftnlen)48);
	svout_(&logfil, nev, &h__[(h_dim1 << 1) + 1], &ndigit, "_saup2: main"
		" diagonal of compressed H matrix", (ftnlen)44);
	i__1 = *nev - 1;
	svout_(&logfil, &i__1, &h__[h_dim1 + 2], &ndigit, "_saup2: subdiagon"
