...
/*     %-------------------------------% */
    ...
    if (msglvl > 0) {
	dvout_(&logfil, n, &h__[(h_dim1 << 1) + 1], &ndigit, "_seigt: main d"
		"iagonal of matrix H", (ftnlen)33);
	if (*n > 1) {
	    i__1 = *n - 1;
	    dvout_(&logfil, &i__1, &h__[h_dim1 + 2], &ndigit, "_seigt: sub d"
    ...
    if (msglvl > 1) {
	dvout_(&logfil, n, &bounds[1], &ndigit, "_seigt: last row of the eig"
