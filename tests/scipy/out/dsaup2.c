	...
	ftnlen bmat_len, ftnlen which_len)
{
    ...
    extern /* Subroutine */ int dseigt_(doublereal *, integer *, doublereal *,
	     integer *, doublereal *, doublereal *, doublereal *, integer *);
    static logical update;
    extern /* Subroutine */ int dsaitr_(integer *, char *, integer *, integer 
	    *, integer *, integer *, doublereal *, doublereal *, doublereal *,
	     integer *, doublereal *, integer *, integer *, doublereal *, 
	    integer *, ftnlen), dsgets_(integer *, char *, integer *, integer 
	    *, doublereal *, doublereal *, doublereal *, ftnlen), dsapps_(
	    integer *, integer *, integer *, doublereal *, doublereal *, 
	    integer *, doublereal *, integer *, doublereal *, doublereal *, 
	    integer *, doublereal *), dsconv_(integer *, doublereal *, 
...
/*           %-----------------------------------------------------% */
	...
	*info = -9999;
    ...
    if (msglvl > 1) {
	dvout_(&logfil, &c__1, &rnorm, &ndigit, "_saup2: Current B-norm of r"
...
/*        %-------------------% */
	...
	ivout_(&logfil, &c__3, kp, &ndigit, "_saup2: NEV, NP, NCONV are", (
		ftnlen)26);
	dvout_(&logfil, &kplusp, &ritz[1], &ndigit, "_saup2: The eigenvalues"
		" of H", (ftnlen)28);
	dvout_(&logfil, &kplusp, &bounds[1], &ndigit, "_saup2: Ritz estimate"
...
/*           %------------------------------------------% */
	...
	if (msglvl > 1) {
	    dvout_(&logfil, &kplusp, &ritz[1], &ndigit, "_saup2: Sorted Ritz"
		    " values.", (ftnlen)27);
	    dvout_(&logfil, &kplusp, &bounds[1], &ndigit, "_saup2: Sorted ri"
...
/*           %---------------------------------------% */
	...
	if (msglvl > 1) {
	    ...
	    ivout_(&logfil, &c__2, kp, &ndigit, "_saup2: NEV and NP are", (
		    ftnlen)22);
	    dvout_(&logfil, nev, &ritz[*np + 1], &ndigit, "_saup2: \"wante"
		    "d\" Ritz values.", (ftnlen)29);
	    dvout_(&logfil, nev, &bounds[*np + 1], &ndigit, "_saup2: Ritz es"
...
/*        %---------------------------------------------------------% */
	...
	ivout_(&logfil, &c__1, np, &ndigit, "_saup2: The number of shifts to"
		" apply ", (ftnlen)38);
	dvout_(&logfil, np, &workl[1], &ndigit, "_saup2: shifts selected", (
		ftnlen)23);
	if (*ishift == 1) {
	    dvout_(&logfil, np, &bounds[1], &ndigit, "_saup2: corresponding "
...
/* L130: */
    ...
    if (msglvl > 2) {
	dvout_(&logfil, &c__1, &rnorm, &ndigit, "_saup2: B-norm of residual "
		"for NEV factorization", (ftnlen)48);
	dvout_(&logfil, nev, &h__[(h_dim1 << 1) + 1], &ndigit, "_saup2: main"
		" diagonal of compressed H matrix", (ftnlen)44);
	i__1 = *nev - 1;
	dvout_(&logfil, &i__1, &h__[h_dim1 + 2], &ndigit, "_saup2: subdiagon"
