...
L1000:
	...
	ivout_(&logfil, &c__1, &j, &ndigit, "_naitr: generating Arnoldi vect"
		"or number", (ftnlen)40);
	dvout_(&logfil, &c__1, rnorm, &ndigit, "_naitr: B-norm of the curren"
...
/*        %-------------------------------------% */
    ...
    if (*(unsigned char *)bmat == 'G') {
	wzdotc_(&z__1, n, &resid[1], &c__1, &workd[ipj], &c__1);
...
L80:
	...
	rtemp[0] = wnorm;
	rtemp[1] = *rnorm;
	dvout_(&logfil, &c__2, rtemp, &ndigit, "_naitr: re-orthogonalization"
		"; wnorm and rnorm are", (ftnlen)49);
	zvout_(&logfil, &j, &h__[j * h_dim1 + 1], &ndigit, "_naitr: j-th col"
		"umn of H", (ftnlen)24);
    }
...
/*        %-----------------------------------------------------% */
	...
	if (msglvl > 2) {
	    rtemp[0] = *rnorm;
	    rtemp[1] = rnorm1;
	    dvout_(&logfil, &c__2, rtemp, &ndigit, "_naitr: iterative refine"
...
/* L110: */
	...
	if (msglvl > 2) {
	    i__1 = *k + *np;
	    i__2 = *k + *np;
	    zmout_(&logfil, &i__1, &i__2, &h__[h_offset], ldh, &ndigit, "_na"
