	...
	integer *ldq, doublereal *workl, doublereal *workd)
{
    ...
    extern /* Subroutine */ int dlacpy_(char *, integer *, integer *, 
	    doublereal *, integer *, doublereal *, integer *), dlaset_(char *,
	     integer *, integer *, doublereal *, doublereal *, doublereal *, 
	    integer *), dlartg_(doublereal *, doublereal *, doublereal *, 
...
/*     %----------------------------------------------% */
	...
	if (msglvl > 2) {
	    ivout_(&logfil, &c__1, &jj, &ndigit, "_napps: shift number.", (
		    ftnlen)21);
	    dvout_(&logfil, &c__1, &sigmar, &ndigit, "_napps: The real part "
		    "of the shift ", (ftnlen)35);
	    dvout_(&logfil, &c__1, &sigmai, &ndigit, "_napps: The imaginary "
...
/* Computing MAX */
		...
		if (msglvl > 0) {
		    ...
		    dvout_(&logfil, &c__1, &h__[i__ + 1 + i__ * h_dim1], &
...
/*     %-------------------------------------% */
    ...
    if (msglvl > 1) {
	dvout_(&logfil, &c__1, &q[kplusp + *kev * q_dim1], &ndigit, "_napps:"
		" sigmak = (e_{kev+p}^T*Q)*e_{kev}", (ftnlen)40);
	dvout_(&logfil, &c__1, &h__[*kev + 1 + *kev * h_dim1], &ndigit, "_na"
	...
	if (msglvl > 2) {
	    dmout_(&logfil, kev, kev, &h__[h_offset], ldh, &ndigit, "_napps:"
