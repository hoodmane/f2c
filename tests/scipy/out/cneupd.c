...
/*     %------------------------------------% */
    ...
    if (msglvl > 2) {
	cvout_(&logfil, ncv, &workl[irz], &ndigit, "_neupd: Ritz values pass"
		"ed in from _NAUPD.", (ftnlen)42);
	cvout_(&logfil, ncv, &workl[ibd], &ndigit, "_neupd: Ritz estimates p"
...
/*        %-------------------------------------% */
	...
	if (msglvl > 2) {
	    cvout_(&logfil, ncv, &workl[irz], &ndigit, "_neupd: Ritz values "
		    "after calling _NGETS.", (ftnlen)41);
	    cvout_(&logfil, ncv, &workl[bounds], &ndigit, "_neupd: Ritz valu"
...
/*        %-------------------------------------------------------% */
	...
	if (msglvl > 1) {
	    cvout_(&logfil, ncv, &workl[iheig], &ndigit, "_neupd: Eigenvalue"
		    "s of H", (ftnlen)24);
	    cvout_(&logfil, ncv, &workl[ihbds], &ndigit, "_neupd: Last row o"
		    "f the Schur vector matrix", (ftnlen)43);
	    if (msglvl > 3) {
		cmout_(&logfil, ncv, ncv, &workl[iuptri], &ldh, &ndigit, 
...
/*           %-----------------------------------------------% */
		...
		*info = 1;
	    ...
	    if (msglvl > 2) {
		cvout_(&logfil, ncv, &workl[iheig], &ndigit, "_neupd: Eigenv"
			"alues of H--reordered", (ftnlen)35);
		if (msglvl > 3) {
		    cmout_(&logfil, ncv, ncv, &workl[iuptri], &ldq, &ndigit, 
...
/*                 %------------------------------------------% */
		...
		i__2 = j;
		wcdotc_(&q__1, &j, &workl[ihbds], &c__1, &workl[invsub + (j - 
			1) * ldq], &c__1);
...
/* L40: */
		...
		ccopy_(&nconv, &workl[invsub + *ncv - 1], &ldq, &workl[ihbds],
			 &c__1);
		cvout_(&logfil, &nconv, &workl[ihbds], &ndigit, "_neupd: Las"
			"t row of the eigenvector matrix for T", (ftnlen)48);
		if (msglvl > 3) {
		    cmout_(&logfil, ncv, ncv, &workl[invsub], &ldq, &ndigit, 
...
/* L60: */
	...
	cvout_(&logfil, &nconv, &d__[1], &ndigit, "_neupd: Untransformed Rit"
		"z values.", (ftnlen)34);
	cvout_(&logfil, &nconv, &workl[ihbds], &ndigit, "_neupd: Ritz estima"
		"tes of the untransformed Ritz values.", (ftnlen)56);
    } else if (msglvl > 1) {
	cvout_(&logfil, &nconv, &d__[1], &ndigit, "_neupd: Converged Ritz va"
		"lues.", (ftnlen)30);
	cvout_(&logfil, &nconv, &workl[ihbds], &ndigit, "_neupd: Associated "
