	...
	integer *info, ftnlen bmat_len)
{
    ...
    static integer jj, ipj, irj, nbx, ivj, ierr, iter;
...
L1000:
    ...
    if (msglvl > 2) {
	ivout_(&logfil, &c__1, &j, &ndigit, "_saitr: generating Arnoldi vect"
		"or no.", (ftnlen)37);
	svout_(&logfil, &c__1, rnorm, &ndigit, "_saitr: B-norm of the curren"
...
/*           %---------------------------------------------------% */
    ...
    if (msglvl > 0) {
	ivout_(&logfil, &c__1, &j, &ndigit, "_saitr: ****** restart at step "
		"******", (ftnlen)37);
    }
...
L80:
	...
	xtemp[0] = wnorm;
	xtemp[1] = *rnorm;
	svout_(&logfil, &c__2, xtemp, &ndigit, "_saitr: re-orthonalization ;"
...
/*        %-----------------------------------------------------% */
    ...
    if (msglvl > 0 && iter > 0) {
	ivout_(&logfil, &c__1, &j, &ndigit, "_saitr: Iterative refinement fo"
		"r Arnoldi residual", (ftnlen)49);
	if (msglvl > 2) {
	    xtemp[0] = *rnorm;
	    xtemp[1] = rnorm1;
	    svout_(&logfil, &c__2, xtemp, &ndigit, "_saitr: iterative refine"
...
/*        %------------------------------------% */
	...
	*ido = 99;
	...
	if (msglvl > 1) {
	    i__1 = *k + *np;
	    svout_(&logfil, &i__1, &h__[(h_dim1 << 1) + 1], &ndigit, "_saitr"
		    ": main diagonal of matrix H of step K+NP.", (ftnlen)47);
	    if (*k + *np > 1) {
		i__1 = *k + *np - 1;
		svout_(&logfil, &i__1, &h__[h_dim1 + 2], &ndigit, "_saitr: s"
