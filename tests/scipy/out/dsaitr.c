...
L1000:
	...
	ivout_(&logfil, &c__1, &j, &ndigit, "_saitr: generating Arnoldi vect"
		"or no.", (ftnlen)37);
	dvout_(&logfil, &c__1, rnorm, &ndigit, "_saitr: B-norm of the curren"
...
L80:
	...
	xtemp[0] = wnorm;
	xtemp[1] = *rnorm;
	dvout_(&logfil, &c__2, xtemp, &ndigit, "_saitr: re-orthonalization ;"
...
/*        %-----------------------------------------------------% */
	...
	if (msglvl > 2) {
	    xtemp[0] = *rnorm;
	    xtemp[1] = rnorm1;
	    dvout_(&logfil, &c__2, xtemp, &ndigit, "_saitr: iterative refine"
...
/*        %------------------------------------% */
	...
	*ido = 99;
	...
	if (msglvl > 1) {
	    i__1 = *k + *np;
	    dvout_(&logfil, &i__1, &h__[(h_dim1 << 1) + 1], &ndigit, "_saitr"
		    ": main diagonal of matrix H of step K+NP.", (ftnlen)47);
	    if (*k + *np > 1) {
		i__1 = *k + *np - 1;
		dvout_(&logfil, &i__1, &h__[h_dim1 + 2], &ndigit, "_saitr: s"
