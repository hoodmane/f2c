...
/*          Note: IPNTR(9:13) is only referenced by cneupd. See Remark 2 below. */
...
/*     of the original problem may be obtained with the ARPACK subroutine cneupd. */

/*  2. If a basis for the invariant subspace corresponding to the converged Ritz */
/*     values is needed, the user must call cneupd immediately following */
...
/*  Cray MPP syntax: */
/*  =============== */
/*  Complex  resid(n), v(ldv,ncv), workd(n,3), workl(lworkl) */
	...
	bmat_len, ftnlen which_len)
{
    ...
    static real tcaup2, tgetv0, tceigh;
    extern real slamch_(char *);
    extern /* Subroutine */ int arscnd_(real *);
    static integer mcaupd, logfil, ndigit;
    static real tcaupd;
    static integer ishift, nitref;
    static real tcaitr, tcgets;
    static integer bounds;
    static real tcapps;
    extern /* Subroutine */ int cstatn_(void);
    static real titref, tcconv;
    static integer msglvl, mxiter, nrorth, nrstrt;
    static real tmvopx;
...
/*     %------------------------------------% */
	...
	*info = 3;
    ...
    if (msglvl > 0) {
	ivout_(&logfil, &c__1, &mxiter, &ndigit, "_naupd: Number of update i"
		"terations taken", (ftnlen)41);
	ivout_(&logfil, &c__1, &np, &ndigit, "_naupd: Number of wanted \"con"
		"verged\" Ritz values", (ftnlen)48);
	cvout_(&logfil, &np, &workl[ritz], &ndigit, "_naupd: The final Ritz "
		"values", (ftnlen)29);
	cvout_(&logfil, &np, &workl[bounds], &ndigit, "_naupd: Associated Ri"
...
/*        %--------------------------------------------------------% */
	...
	do_fio(&c__1, (char *)&tcaupd, (ftnlen)sizeof(real));
	do_fio(&c__1, (char *)&tcaup2, (ftnlen)sizeof(real));
	do_fio(&c__1, (char *)&tcaitr, (ftnlen)sizeof(real));
	do_fio(&c__1, (char *)&titref, (ftnlen)sizeof(real));
	do_fio(&c__1, (char *)&tgetv0, (ftnlen)sizeof(real));
	do_fio(&c__1, (char *)&tceigh, (ftnlen)sizeof(real));
	do_fio(&c__1, (char *)&tcgets, (ftnlen)sizeof(real));
	do_fio(&c__1, (char *)&tcapps, (ftnlen)sizeof(real));
	do_fio(&c__1, (char *)&tcconv, (ftnlen)sizeof(real));
