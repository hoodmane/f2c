	...
	real *resid, real *q, integer *ldq, real *workl, real *workd)
{
    ...
    extern /* Subroutine */ int sscal_(integer *, real *, real *, integer *), 
	    ...
	    real *, integer *, real *), sgemv_(char *, integer *, integer *, 
    ...
    extern /* Subroutine */ int slacpy_(char *, integer *, integer *, real *, 
	    integer *, real *, integer *), slaset_(char *, integer *, integer 
	    *, real *, real *, real *, integer *), slartg_(real *, real *, 
	    real *, real *, real *);
    static integer mnapps, msglvl, istart;
    static real tnapps, smlnum;
...
/*     %----------------------------------------------% */
	...
	if (msglvl > 2) {
	    ivout_(&logfil, &c__1, &jj, &ndigit, "_napps: shift number.", (
		    ftnlen)21);
	    svout_(&logfil, &c__1, &sigmar, &ndigit, "_napps: The real part "
		    "of the shift ", (ftnlen)35);
	    svout_(&logfil, &c__1, &sigmai, &ndigit, "_napps: The imaginary "
...
/*           %----------------------------------------% */
	    ...
	    tst1 = (r__1 = h__[i__ + i__ * h_dim1], abs(r__1)) + (r__2 = h__[
		    i__ + 1 + (i__ + 1) * h_dim1], abs(r__2));
	    if (tst1 == 0.f) {
		i__3 = kplusp - jj + 1;
...
/* Computing MAX */
	    r__2 = ulp * tst1;
	    if ((r__1 = h__[i__ + 1 + i__ * h_dim1], abs(r__1)) <= max(r__2,
		    smlnum)) {
		if (msglvl > 0) {
		    ivout_(&logfil, &c__1, &i__, &ndigit, "_napps: matrix sp"
			    "litting at row/column no.", (ftnlen)42);
		    ivout_(&logfil, &c__1, &jj, &ndigit, "_napps: matrix spl"
			    "itting with shift number.", (ftnlen)43);
		    svout_(&logfil, &c__1, &h__[i__ + 1 + i__ * h_dim1], &
...
L40:
	...
	if (msglvl > 2) {
	    ivout_(&logfil, &c__1, &istart, &ndigit, "_napps: Start of curre"
		    "nt block ", (ftnlen)31);
	    ivout_(&logfil, &c__1, &iend, &ndigit, "_napps: End of current b"
		    "lock ", (ftnlen)29);
	}
...
/*        %------------------------------------------------------% */
	...
	if (istart + 1 == iend && abs(sigmai) > 0.f) {
	    goto L100;
	}
...
/*        %--------------------------------------------% */
	...
	tst1 = (r__1 = h__[i__ + i__ * h_dim1], abs(r__1)) + (r__2 = h__[i__ 
		+ 1 + (i__ + 1) * h_dim1], abs(r__2));
	if (tst1 == 0.f) {
	    tst1 = slanhs_("1", kev, &h__[h_offset], ldh, &workl[1]);
	}
/* Computing MAX */
	r__1 = ulp * tst1;
	if (h__[i__ + 1 + i__ * h_dim1] <= max(r__1,smlnum)) {
	    h__[i__ + 1 + i__ * h_dim1] = 0.f;
	}
/* L130: */
    }
...
/*     %-------------------------------------% */
    ...
    if (msglvl > 1) {
	svout_(&logfil, &c__1, &q[kplusp + *kev * q_dim1], &ndigit, "_napps:"
		" sigmak = (e_{kev+p}^T*Q)*e_{kev}", (ftnlen)40);
	svout_(&logfil, &c__1, &h__[*kev + 1 + *kev * h_dim1], &ndigit, "_na"
		"pps: betak = e_{kev+1}^T*H*e_{kev}", (ftnlen)37);
	ivout_(&logfil, &c__1, kev, &ndigit, "_napps: Order of the final Hes"
		"senberg matrix ", (ftnlen)45);
	if (msglvl > 2) {
	    smout_(&logfil, kev, kev, &h__[h_offset], ldh, &ndigit, "_napps:"
